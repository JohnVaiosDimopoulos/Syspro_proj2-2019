...
class Sender {
 private:
  ...
  void Recursively_go_Through_dir_and_send(const char *current_path_relative_to_input_dir,const char *current_dir_full_path,Log_file_handler *log_file_handler);
  ...
  void Send_through_pipe(const char *full_file_path,const char *file_path_relative_to_input,Log_file_handler *log_file_handler);
  void Read_from_file_and_write_in_pipe(const int &input_file_fd, const u_int32_t &file_size);
  int Calculate_new_buffer_size(const u_int32_t &bytes_left_to_send, int buffer_size);
  void Mark_end_of_input();
 ...
 public:
  ...
  //==API==//
  int Send_data(const int &client_id, const Argument_data &data, Log_file_handler log_file_handler);
